...
import { exportMarkdownNote, exportPdfDocument, importMarkdownNote } from "./api";
...
describe("importExport api", () => {
  ...
  test("exports generated PDF data to the selected path", async () => {
    mockedSave.mockResolvedValue("D:\\exports\\读书笔记.pdf");
    mockedInvoke.mockResolvedValue(undefined);

    await expect(exportPdfDocument("读书笔记", "JVBERi0xLjQ=")).resolves.toBe(true);

    expect(save).toHaveBeenCalledWith({
      defaultPath: "读书笔记.pdf",
      filters: [{ name: "PDF", extensions: ["pdf"] }],
    });
    expect(invoke).toHaveBeenCalledWith("notes_export_pdf", {
      path: "D:\\exports\\读书笔记.pdf",
