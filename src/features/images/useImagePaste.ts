...
export async function saveImageFile(file: File, noteId: string, t?: TFunction): Promise<string> {
  ...
  if (!ext) {
    throw new Error(
      t?.("errors.unsupportedImageFormat", { defaultValue: "不支持的图片格式" }) ??
        "不支持的图片格式",
    );
  }
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        for (const file of files) {
          const relativePath = await saveImageFile(file, resolvedId, t);
