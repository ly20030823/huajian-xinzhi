...
function CodeBlock({ children, language }: { children: React.ReactNode; language?: string }) {
  ...
  return (
    <div className="markdown-code-block my-3 group">
      ...
      <pre className="markdown-code-scroll">
        <code>
          {lines.map((line, index) => (
            <span className="markdown-code-line" data-line={index + 1} key={`${index}-${line}`}>
              {line || " "}
            </span>
          ))}
        </code>
...
const staticComponents: Components = {
  ...
  h4: ({ children, id }) => (
    <h4 id={id} className="text-[1em] font-display font-bold text-ink mt-4 mb-2 tracking-wide">
      {children}
    </h4>
  ),
  h5: ({ children, id }) => (
    <h5 id={id} className="text-[0.96em] font-display font-bold text-ink mt-4 mb-2 tracking-wide">
      {children}
    </h5>
  ),
  h6: ({ children, id }) => (
    <h6 id={id} className="text-[0.92em] font-display font-bold text-ink mt-4 mb-2 tracking-wide">
      {children}
    </h6>
  ),
    ...
    if (language.toLowerCase() === "mermaid") {
      return (
        <div className="mermaid-preview-card">
          <MermaidDiagram code={extractText(children).replace(/\n$/, "")} />
        </div>
      );
    }
  ...
  table: ({ children }) => (
    <div className="markdown-table-scroll my-3">
      <table className="markdown-rendered-table">{children}</table>
    </div>
  ),
  th: ({ children }) => <th className="markdown-rendered-table__header">{children}</th>,
  td: ({ children }) => <td className="markdown-rendered-table__cell">{children}</td>,
  input: ({ checked, ...props }) => (
    <input
      {...props}
      checked={checked}
      disabled
      className="mr-2 inline-block h-[1.05em] w-[1.05em] align-[-0.12em] accent-bamboo"
    />
  ),
};
...
export function MarkdownPreview({
  ...
  return (
    <div
      className="font-body markdown-selectable markdown-preview"
