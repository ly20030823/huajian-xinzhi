...
export function NotepadOpenPanel({ notes, onOpenNote, onCreateNew }: NotepadOpenPanelProps) {
    ...
    <div className="flex flex-col flex-1 min-h-0">
      <div className="px-3 pt-2 pb-1.5 shrink-0">
        <div className="flex items-center gap-1.5">
          <div className="flex flex-1 min-w-0 items-center gap-2 px-2.5 py-1.5 rounded-lg bg-paper-warm/60 border border-paper-deep/30">
            <svg
              width="13"
              height="13"
              viewBox="0 0 24 24"
            ...
            <input
              ...
              className="min-w-0 flex-1 text-[12px] font-body text-ink placeholder:text-ink-ghost/60 bg-transparent"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <button
            type="button"
            data-testid="notepad-new-from-open"
            onClick={onCreateNew}
            aria-label={t("notepad.button.new", { defaultValue: "新建便签" })}
            title={t("notepad.button.new", { defaultValue: "新建便签" })}
            className="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg border border-bamboo/20 bg-bamboo-mist/45 text-bamboo hover:bg-bamboo-mist transition-all cursor-pointer"
          >
            <svg
              width="13"
              height="13"
