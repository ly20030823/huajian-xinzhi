...
interface SettingsPanelProps {
  config: AppConfig;
  showWritingPet: boolean;
  onChange: (config: AppConfig) => void;
  onWritingPetVisibilityChange: (visible: boolean) => void;
  onOpenDataDir: () => void;
  onClose: () => void;
}
...
export function SettingsPanel({
  ...
  const themeOptions = useMemo<Array<{ value: ThemeOption; label: string }>>(
    () => [
      ...
      { value: "white", label: t("settings.theme.white", { defaultValue: "纯白" }) },
    ...
    >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hidden px-4 py-4 space-y-5">
        <section className="space-y-2">
          ...
          />
          <ToggleRow
            label={t("settings.showWritingPet", { defaultValue: "显示橘猫陪伴" })}
            checked={showWritingPet}
            onChange={onWritingPetVisibilityChange}
          />
          <p className="text-[10px] leading-relaxed text-ink-faint/80">
            {t("settings.showWritingPetHint", {
              defaultValue: "双击橘猫可快速隐藏；隐藏后双击标题右侧空白处即可恢复。",
            })}
          </p>
        </section>
        ...
        <section className="space-y-2">
          ...
          <p className="text-[10px] leading-relaxed text-ink-faint/80">
            {t("settings.dataDirHint", {
              defaultValue:
                "所选文件夹就是工作区；Markdown 保持原位，只会新增 metadata.json 用于记录标题和顺序。",
            })}
          </p>
        </section>
        ...
        <section className="space-y-2">
          <ToggleRow
            label={t("settings.defaultView.showSource", {
              defaultValue: "默认显示 Markdown 源码",
            })}
            checked={config.defaultViewMode === "content-source"}
            onChange={(checked) =>
              setConfigValue("defaultViewMode", checked ? "content-source" : "content")
            }
          />
          <p className="px-1 text-[10px] leading-relaxed text-ink-ghost/75">
            {t("settings.defaultView.hint", {
              defaultValue: "内容编辑支持 Markdown 快捷输入，源码面板可随时打开。",
            })}
          </p>
        </section>
...
function ToggleRow({ label, checked, onChange }: ToggleRowProps) {
  return (
    <button
      ...
      className="w-full flex items-center justify-between h-9 rounded-lg px-2.5 bg-paper-warm/45 border border-paper-deep/25 cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-bamboo/35"
    >
      <span className="text-[12px] text-ink-soft">{label}</span>
      <span
        aria-hidden="true"
        className={`relative block h-[18px] w-8 shrink-0 rounded-full transition-colors duration-250 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          checked ? "bg-bamboo" : "bg-paper-deep/50"
        }`}
      >
        <span
          className={`absolute left-[2px] top-[2px] block h-[14px] w-[14px] rounded-full bg-white shadow-[0_1px_2px_rgba(0,0,0,0.15)] transition-transform duration-250 ease-[cubic-bezier(0.22,1,0.36,1)] ${
