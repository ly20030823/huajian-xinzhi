...
export function NotePad({
  ...
  const switchSurfaceMode = useCallback(
    async (nextMode: NoteSurfaceMode) => {
      ...
      try {
        ...
        await setCurrentWindowAlwaysOnTop(shouldSurfaceStayOnTop(nextMode));
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      if ((event.ctrlKey || event.metaKey) && event.key === "s") {
        event.preventDefault();
        if (editingNoteId && !isEditingUnlocked) return;
        void handleSave();
      }
    }
  ...
  }, [editingNoteId, handleSave, isEditingUnlocked]);
  ...
  const isExistingNoteLocked = shouldLockExistingNote(editingNoteId, isEditingUnlocked);
      ...
      {isTile ? (
        ...
        >
          <div className="absolute top-2 right-2 z-10 flex items-center gap-0.5">
            <button
              type="button"
              data-testid="tile-return-to-pad"
              aria-label={t("notepad.tooltip.returnToPad", { defaultValue: "返回便签编辑" })}
              title={t("notepad.tooltip.returnToPad", { defaultValue: "返回便签编辑" })}
              onMouseDown={(event) => event.stopPropagation()}
              onClick={(event) => {
                event.stopPropagation();
                void switchSurfaceMode("pad");
              }}
              className="w-6 h-6 flex items-center justify-center rounded-full text-ink-ghost/70 hover:text-bamboo hover:bg-cloud/55 transition-colors cursor-pointer"
            >
              <svg
                width="12"
                height="12"
            ...
            <button
              type="button"
              aria-label={t("notepad.tooltip.closeTile", { defaultValue: "关闭磁贴" })}
              title={t("notepad.tooltip.closeTile", { defaultValue: "关闭磁贴" })}
              onMouseDown={(event) => event.stopPropagation()}
              onClick={(event) => {
                event.stopPropagation();
                void handleClose();
          ...
          <>
            ...
            >
              ...
              <div className="ml-auto flex items-center gap-1.5">
                {mode === "new" && editingNoteId && (
                  <button
                    type="button"
                    role="switch"
                    aria-checked={isEditingUnlocked}
                    data-testid="notepad-edit-lock"
                    onMouseDown={(event) => event.stopPropagation()}
                    onClick={() => {
                      const nextUnlocked = !isEditingUnlocked;
                      setIsEditingUnlocked(nextUnlocked);
                      if (nextUnlocked) {
                    ...
                    aria-label={
                      isEditingUnlocked
                        ? t("notepad.editLock.lockTitle", { defaultValue: "锁定，防止误修改" })
                        : t("notepad.editLock.unlockTitle", { defaultValue: "解锁后才能修改" })
                    }
                    className={`group w-7 h-7 flex items-center justify-center rounded-lg border transition-all duration-200 cursor-pointer ${
                      isEditingUnlocked
                        ? "border-bamboo/30 bg-bamboo/10 text-bamboo hover:bg-bamboo/15"
                        : "border-paper-deep/70 bg-paper-warm/70 text-ink-faint hover:text-ink-soft hover:bg-paper-warm"
                    }`}
                    title={
                      isEditingUnlocked
                        ? t("notepad.editLock.lockTitle", { defaultValue: "锁定，防止误修改" })
            ...
            {mode === "new" ? (
              ...
              >
                <input
                  ...
                  type="text"
                  value={title}
                  readOnly={isExistingNoteLocked}
                  aria-readonly={isExistingNoteLocked}
                  onChange={(event) => {
                  ...
                  className={`w-full font-display font-medium text-ink placeholder:text-ink-ghost/60 mb-2 tracking-wide shrink-0 transition-opacity ${
                    isExistingNoteLocked ? "cursor-default" : ""
                  }`}
                ...
                <textarea
                  ...
                  onPaste={isExistingNoteLocked ? undefined : imagePasteHandler}
                  onDrop={isExistingNoteLocked ? undefined : imageDropHandler}
                  onDragOver={isExistingNoteLocked ? undefined : imageDragOverHandler}
                  onKeyDown={(event) => {
                  ...
                  className={`w-full flex-1 min-h-0 pb-2 leading-relaxed text-ink-soft font-body placeholder:text-ink-ghost/50 transition-opacity ${
                    isExistingNoteLocked ? "cursor-text" : ""
                  }`}
                ...
                <div className="flex items-center justify-between mt-auto pt-2 border-t border-paper-deep/30 shrink-0">
                  <span className="text-[11px] text-ink-ghost font-mono tabular-nums truncate max-w-[170px]">
                    {`${countNoteChars(content)} ${t("common.wordCountUnit", { defaultValue: "字" })} · ${
                      isExistingNoteLocked
                        ? t("notepad.editLock.readOnly", { defaultValue: "只读" })
                        : statusLabel[status]
                    }`}
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      data-testid="notepad-new-from-editor"
                      onClick={resetDraft}
                      className="px-4 py-1.5 text-[12px] text-ink-faint hover:text-bamboo rounded-lg hover:bg-bamboo-mist/50 transition-all duration-200 cursor-pointer"
                    >
                      {t("notepad.button.new", { defaultValue: "新建" })}
                    </button>
                    <button
                      onClick={() => void handleSave()}
                      disabled={isExistingNoteLocked}
                      className="px-4 py-1.5 text-[12px] text-cloud bg-bamboo hover:bg-bamboo-light rounded-lg transition-all duration-200 font-medium cursor-pointer disabled:bg-paper-deep disabled:text-ink-ghost disabled:cursor-not-allowed"
                    >
