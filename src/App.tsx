...
function App() {
  ...
  useEffect(() => {
    const handleEditorShortcut = (event: KeyboardEvent) => {
      ...
      if (key !== "f" && key !== "h" && key !== "r") return;

      // WebView has its own find/reload accelerators. Intercept them at the window boundary
      // before they can open the native search box or reload the application.
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      window.dispatchEvent(
        new CustomEvent("floral-editor-find", {
          detail: { replace: key === "h" || key === "r" },
        }),
      );
    };
  ...
  return (
    <ContextMenuProvider>
      <div className="app-window-shell h-screen min-h-0 font-body text-ink overflow-clip">
        {activeView === "main" ? (
          <MainWindow />
        ) : activeView === "notepad" ? (
